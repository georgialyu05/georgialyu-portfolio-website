...
function buildGrid() {
  ...
  for (let i = 0; i < total; i++) {
    ...
    if (blackSet.has(i)) {
      ...
      imgEl.src = `public/homepage-interaction/characters/${charKey}.jpg`;
      imgEl.alt = '';
...
// ── Single-row grid (grid2) ──
const GRID2_CHAR = '3';
const GRID2_MESSAGE = 'Product Designer & Design Engineer.';

function buildGrid2() {
  const el = document.getElementById('grid2');
  if (!el) return;
  const COLS2 = getLayout().cols;
  const TOTAL2 = COLS2;
  const center2 = Math.floor(COLS2 / 2);

  // Fixed black cells: 2 random positions for the side characters
  const blackSet2 = new Set([center2]);
  while (blackSet2.size < 3) {
    blackSet2.add(Math.floor(Math.random() * TOTAL2));
  }

  // Center gets GRID2_CHAR; the other two get fixed bottom-character images
  const bottomChars = ['b1', 'b2'];
  const imgMap2 = new Map([[center2, GRID2_CHAR]]);
  let idx2 = 0;
  for (const idx of blackSet2) {
    if (idx !== center2) imgMap2.set(idx, bottomChars[idx2++]);
  }
  ...
  for (let i = 0; i < TOTAL2; i++) {
    const cell = document.createElement('div');
    cell.className = 'cell' + (blackSet2.has(i) ? ' black' : '') + (i === center2 ? ' open' : '');
    ...
    if (blackSet2.has(i)) {
      ...
      imgWrap.className = 'cell-img';
      const imgEl = document.createElement('img');
      const isBottomChar = charKey === 'b1' || charKey === 'b2';
      imgEl.src = isBottomChar
        ? `public/homepage-interaction/buttom-characters/${charKey.slice(1)}.jpg`
        : `public/homepage-interaction/characters/${charKey}.jpg`;
      imgEl.alt = '';
      imgWrap.appendChild(imgEl);
      cell.appendChild(imgWrap);
      ...
      const bubble = document.createElement('div');
      bubble.className = 'bubble';
      bubble.textContent = i === center2 ? GRID2_MESSAGE : (charMessages[charKey] || '');
      cell.appendChild(bubble);
